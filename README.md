...
| Режим | Где ставить софт | Когда использовать |
|-------|------------------|--------------------|
| **SSH** | Только на master (ключ SSH) | Master может подключиться к ноде по SSH |
| **Agent** | На удалённой ноде (`backup-agent`) | Нода за NAT, закрытый SSH, push-модель |
